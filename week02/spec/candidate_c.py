"""
Candidate C for the brief in BRIEF.md.

    uv run candidate_c.py

Prints one row per row of the grid and writes candidate_c.svg.
...
def place(rng, row):
    ...
    dx = rng.uniform(-1, 1) * hurt * SQUARE
    dy = rng.uniform(-1, 1) * hurt * SQUARE
    return angle, dx, dy
...
def main():
    ...
    with open("candidate_c.svg", "w", encoding="utf-8") as handle:
        handle.write(svg)
    print("wrote candidate_c.svg")
